...
"""
#自动下载高星项目 https://codeload.github.com/nytimes/covid-19-data/zip/master
#监控和邮件分离
#记录上一次脚本启动、数据库更新
"""
...
class Githublook():
    ...
    # 获取关键字从开始日期到结束日期的数据
    def github_date2sqlite3(self, s, e):
        self.sqlite3_create()
    ...
    def readini(self, inipath="mail.ini"):  # 为配合独立exe免编译读取配置文件写的函数
        config = configparser.ConfigParser()
        config.read(inipath, encoding='utf-8')
        self._mail_host = config.get('mail', 'host')
        self._mail_user = config.get('mail', 'user')
        self._mail_pass = config.get('mail', 'passwd')
        self._to_reciver = list(config.get('mail', 'to').split(' '))
        self._look_sleep = int(config.get('mail', 'sleep')) * 60
...
if __name__ == '__main__':
    ...
    _g = Githublook(['cve-20', ])  # 多个关键字监控 mailpath = os.getcwd() + '\\mail.ini'
    mailpath = os.getcwd() + '\\mail.ini'
